...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        pruning_decision_layers: Optional[List[int]] = None,
        ...
        token_importance_scores = None  # 用于累积注意力得分
        original_seq_len = inputs_embeds.shape[1]
        alive_token_indices = torch.arange(original_seq_len, device=inputs_embeds.device)
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                ...
                importance_this_layer = attention_weights.sum(dim=(1, 2))
                if token_importance_scores is None:
                    token_importance_scores = torch.zeros_like(importance_this_layer)
                token_importance_scores += importance_this_layer

                if layer_idx + 1 in pruning_decision_layers:
                    current_is_image_token_mask = is_image_token_mask[alive_token_indices]
                    importance_scores_for_pruning = token_importance_scores.clone()
                    importance_scores_for_pruning[:, ~current_is_image_token_mask] = torch.finfo(importance_scores_for_pruning.dtype).max
                    num_alive_image_tokens = current_is_image_token_mask.sum().item()
                    num_image_tokens_to_prune = int(num_alive_image_tokens * pruning_ratio)
                    if num_image_tokens_to_prune > 0:
                        _, indices_to_prune = torch.topk(token_importance_scores, num_image_tokens_to_prune, dim=1, largest=False)
                        keep_mask = torch.ones_like(token_importance_scores, dtype=torch.bool)
                        keep_mask[indices_to_prune] = False
                        for i in range(len(hidden_states.shape[0])):
                            keep_mask_for_batch = keep_mask[i]
                            hidden_states = hidden_states[:, keep_mask_for_batch, :]
                            if use_cache and next_decoder_cache is not None:
                                next_decoder_cache.prune_tokens(keep_mask_for_batch)  # 假设Cache对象有这样一个方法
                            causal_mask = causal_mask[:, :, keep_mask_for_batch, :][:, :, :, keep_mask_for_batch]
                            token_importance_scores = token_importance_scores[keep_mask_for_batch]
                            alive_token_indices = alive_token_indices[keep_mask_for_batch]
                            # position_ids, position_embeddings 也可能需要更新
                        print(
                            f"Layer {layer_idx + 1}: Pruned {num_image_tokens_to_prune} tokens. Remaining: {hidden_states.shape[1]}")
...
def standard_dpc(candidate_tokens: torch.Tensor, budget: int, n_neighbors: int = 7) -> torch.Tensor:
    """
    Standard Density Peak Clustering (DPC) for token selection.
    """
...
def select_supplementary_tokens(
    ...
    """
    Two-stage token processing: Selection & Aggregation.
    """
    ...
    for frame_idx in range(num_frames):
        ...
        supplementary_budget = max(0, min(total_budget - len(must_select_indices), len(candidate_indices)))
        ...
        key_indices = torch.cat([must_select_indices, supplementary_indices]).sort().values

        # 3. Fast Vectorized Aggregation
        if use_aggregation and key_indices.numel() > 0:
            ...
            # Find nearest key token for each discarded token
            ...
            aggregated_discarded_sum = torch.zeros_like(key_features)
...
def calculate_dynamic_token_budgets(
    patch_features_per_frame: torch.Tensor,
    question_embedding: torch.Tensor,
    total_retention_ratio: float = 0.25,
    r_min_ratio: float = 0.10,
    tau_similarity_threshold: float = 0.95
):
    """
    Calculate dynamic token budgets based on visual content and question relevance.
    """
    device = patch_features_per_frame.device
    num_frames, tokens_per_frame, _ = patch_features_per_frame.shape

    # 1. Segment Generation
    frame_features = F.normalize(torch.mean(patch_features_per_frame, dim=1), p=2, dim=1)
    ...
    # Merge single-frame segments
    i = 0
    while i < len(segments):
        if len(segments[i]) == 1:
            current_segment_feature = frame_features[segments[i][0]]
            sim_left, sim_right = -1, -1
    ...
    # 2. Relevance Scoring
    segment_features = torch.stack([torch.mean(frame_features[seg], dim=0) for seg in segments])
    relevance_scores = torch.matmul(
        F.normalize(segment_features, p=2, dim=1), 
        F.normalize(question_embedding.squeeze(), p=2, dim=0)
    )
    weights = F.softmax(relevance_scores, dim=0)

    # 3. Base Allocation
    target_total_tokens = (num_frames * tokens_per_frame) * total_retention_ratio
    min_tokens_per_frame = int(round(tokens_per_frame * r_min_ratio))
    final_frame_budgets = [min_tokens_per_frame] * num_frames
    
    tokens_to_distribute = int(max(0, target_total_tokens - sum(final_frame_budgets)))

    # 4. Iterative Redistribution
    segment_capacities = torch.tensor(
        [len(seg) * (tokens_per_frame - min_tokens_per_frame) for seg in segments], 
    ...
    while tokens_to_distribute > 0:
        ...
        if total_eligible_weight <= 1e-8:
            normalized_eligible_weights = eligible_mask.float() / torch.sum(eligible_mask).item()
        else:
        ...
        actual_allocations = torch.min(tentative_allocations, segment_capacities - allocated_extra_tokens)
        ...
        if distributed_this_round == 0 and tokens_to_distribute > 0:
            ...
            if torch.any(eligible_mask):
                winner_idx = torch.argmax(weights * eligible_mask.float()).item()
                add_amount = int(min(tokens_to_distribute, (segment_capacities[winner_idx] - allocated_extra_tokens[winner_idx]).item()))
    ...
    for seg_idx, extra_tokens in enumerate(allocated_extra_tokens):
        if len(segments[seg_idx]) > 0:
            extra_per_frame = int(round(extra_tokens.item() / len(segments[seg_idx])))
            for frame_idx in segments[seg_idx]:
                final_frame_budgets[frame_idx] = min(tokens_per_frame, final_frame_budgets[frame_idx] + extra_per_frame)
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def _process_dynamic_video_tokens(
        self,
        video_embeds,
        video_grid_thw,
        input_ids,
        ...
        t_dim = video_grid_thw[0, 0].item()
        h_dim = video_grid_thw[0, 1].item() // 2
        w_dim = video_grid_thw[0, 2].item() // 2
        num_patches_per_frame = h_dim * w_dim
        hidden_dim = video_embeds.shape[-1]
        target_device = video_embeds.device

        video_embeds_reshaped = video_embeds.view(t_dim, num_patches_per_frame, hidden_dim)

        dynamic_budget_tokens, segments = calculate_dynamic_token_budgets(
            patch_features_per_frame=video_embeds_reshaped,
            question_embedding=question_embedding,
            total_retention_ratio=float(os.environ.get("RETENTION_RATIO", 0.25)),
            r_min_ratio=float(os.environ.get("R_MIN_RATIO", 0.10)),
        )

        attention_map_all = None
        if blip_future is not None:
            attention_map_all = blip_future.result()
            blip_executor.shutdown(wait=False)

        all_pruning_indices = []
        if attention_map_all is not None:
            attention_map_all = attention_map_all.to(target_device)
            num_frames_blip = attention_map_all.shape[0]
            for i in range(t_dim):
                if i >= num_frames_blip:
                    all_pruning_indices.append(torch.tensor([], device=target_device, dtype=torch.long))
                    continue

                raw_budget = int(dynamic_budget_tokens[i] * doimain_rate)
                if raw_budget <= 0:
                    all_pruning_indices.append(torch.tensor([], device=target_device, dtype=torch.long))
                    continue

                scores = attention_map_all[i]
                k_safe = min(raw_budget, scores.shape[0])
                _, top_indices = torch.topk(scores, k=k_safe, dim=-1)
                all_pruning_indices.append(top_indices)
        else:
            for _ in range(t_dim):
                all_pruning_indices.append(torch.tensor([], device=target_device, dtype=torch.long))

        final_frame_data = select_supplementary_tokens(
            patch_features_per_frame=video_embeds_reshaped,
        ...
        use_aggr = os.environ.get("USE_AGGREGATION", "False") == "True"
        for i in range(t_dim):
            frame_data = final_frame_data.get(i)
            if frame_data is None: continue
            final_feats, final_inds = frame_data
            if final_inds.numel() == 0: continue
            
            if use_aggr:
                video_embeds_reshaped[i][final_inds] = final_feats.to(video_embeds_reshaped.dtype)

        video_mask = (input_ids == self.config.video_token_id)
        video_start = video_mask[0].float().argmax().item()
        video_end = video_mask.shape[1] - video_mask[0].flip(0).float().argmax().item()
        seq_len = input_ids.shape[1]

        indices_to_keep = [torch.arange(0, video_start, device=target_device)]
        current_frame_offset = video_start

        for i in range(t_dim):
            frame_data = final_frame_data.get(i)
            if frame_data is not None:
                _, frame_kept_relative = frame_data
                if frame_kept_relative.numel() > 0:
                    frame_kept_global = current_frame_offset + frame_kept_relative
                    indices_to_keep.append(frame_kept_global)
            current_frame_offset += num_patches_per_frame

        indices_to_keep.append(torch.arange(video_end, seq_len, device=target_device))
        
        final_indices = torch.cat(indices_to_keep).sort().values

        new_inputs_embeds = inputs_embeds[:, final_indices, :]
        new_attention_mask = attention_mask[:, final_indices] if attention_mask is not None else None
        
        new_position_ids = None
        if position_ids is not None:
            new_position_ids = position_ids[:, :, final_indices]

        new_cache_position = cache_position
        if cache_position is not None:
            if cache_position.dim() == 1:
                new_cache_position = cache_position[final_indices]
            else:
                new_cache_position = cache_position[:, final_indices]
    ...
    def forward(
        ...
        feature_model: Optional[torch.nn.Module] = None,
        raw_images: Optional[torch.Tensor] = None,
        doimain_rate: Optional[torch.FloatTensor] = None,
        ...
        if input_ids.shape[1]!=1 and feature_model is not None and raw_images is not None and video_grid_thw is not None:
            def run_blip_background():
                video_frames = raw_images[0] if isinstance(raw_images[0], list) else raw_images
                return feature_model.extract_relevant_patch_indices_dynamic_no_crop(
                    video_frames_list=raw_images,
                    question_texts=questions,
                    thw_grid=video_grid_thw[0],
                )
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                inputs_embeds = inputs_embeds.masked_scatter(mask_expanded, image_embeds.to(inputs_embeds.device, inputs_embeds.dtype))

            if pixel_values_videos is not None:
                video_embeds = self.get_video_features(pixel_values_videos, video_grid_thw)
                mask = input_ids == self.config.video_token_id
                mask_expanded = mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
                inputs_embeds = inputs_embeds.masked_scatter(mask_expanded, video_embeds.to(inputs_embeds.device, inputs_embeds.dtype))
        ...
        if feature_model is not None and input_ids.shape[1] != 1 and video_grid_thw is not None and video_embeds is not None:
            inputs_embeds, attention_mask, position_ids, cache_position = self._process_dynamic_video_tokens(
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    #     is_prefill_step = cache_position is not None and cache_position.shape[-1] > 1
    #     if is_prefill_step and hasattr(outputs, "attention_mask") and outputs.attention_mask is not None:
