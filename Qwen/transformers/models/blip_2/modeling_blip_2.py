...
class QueryGenerator(nn.Module):
    def __init__(self, vit_hidden_dim=1408, qformer_query_dim=768, num_queries=32):
        super().__init__()
        self.num_queries = num_queries
        self.qformer_query_dim = qformer_query_dim

        # 一个简单的两层MLP
        self.mlp = nn.Sequential(
            nn.Linear(vit_hidden_dim, 2048), # 扩大维度以增加表达能力
            nn.GELU(),
            nn.LayerNorm(2048),
            nn.Linear(2048, num_queries * qformer_query_dim) # 输出所有查询向量拼接后的大小
        )

    def forward(self, cls_token_embedding):

        flat_queries = self.mlp(cls_token_embedding)
        dynamic_queries = flat_queries.view(
            -1, self.num_queries, self.qformer_query_dim
        )
...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel, GenerationMixin):
    ...
    def get_new_query_embeddings(self, cls_token_embedding):
        return self.query_generator(cls_token_embedding)
    ...
    def get_image_features(
        ...
        # step 1: forward the images through the vision encoder,
        # to get image embeddings of shape (batch_size, seq_len, hidden_size)
        embeddings.to(device=pixel_values.device)
        vision_outputs = self.vision_model(
            pixel_values=pixel_values,
            interpolate_pos_encoding=interpolate_pos_encoding,
            return_dict=True,
        ...
        # cls_feature = image_embeds[:, 0, :]
        # query_embeddings = self.get_new_query_embeddings(cls_feature)

        # step 2: forward the query tokens through the QFormer, using the image embeddings for cross-attention
        image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device)

        query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)
        question_embeds = embeddings(
            input_ids= question_id,
            query_embeds=query_tokens,
            # position_ids=position_ids,
        )
        target_dtype = self.qformer.layernorm.weight.dtype
