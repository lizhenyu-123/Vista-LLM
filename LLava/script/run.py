...
def prepare_model_inputs(prompt, question, tokenizer, model, device):

    final_input_text = DEFAULT_IMAGE_TOKEN + '\n' + prompt
    conv = copy.deepcopy(conv_templates["qwen_1_5"])
    conv.append_message(conv.roles[0], final_input_text)
    conv.append_message(conv.roles[1], None)
    prompt_for_tokenizer = conv.get_prompt()
    
    input_ids = tokenizer_image_token(prompt_for_tokenizer, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").to(device)
    
    question_ids = tokenizer(question, return_tensors="pt").input_ids.to(device)
    question_word_embeddings = model.get_model().embed_tokens(question_ids)
    question_sentence_embedding = question_word_embeddings[0].mean(dim=0)
    
    return input_ids, question_sentence_embedding

def generate(args):
    logging.info("--- Phase 1: Loading Models ---")
    tokenizer, model, image_processor, max_length = load_pretrained_model(
        args.model_path, None, "llava_qwen", torch_dtype="bfloat16", device_map="cuda:0"
    )
    
    feature_model = None
    if args.use_way: 
        logging.info("Dynamic token way is enabled. Loading BLIP2 model.")
        feature_model = Blip2RegionExtractor(train_path=args.train_path, device_map="cuda:1")
    ...
    if args.dataset == 'videomme':
        ...
        prefix, json_path, _, _ = data_list[args.task_type]
    ...
    with torch.no_grad():
        ...
        for batch in progress_bar:
                ...
                try:
                    if args.use_preextracted_frames:
                        ...
                        if args.dataset == 'videomme':
                            ...
                            frame_indices, original_fps = None, None
                        elif args.dataset == 'longvideobench':
                    ...
                    if not (args.use_preextracted_frames and os.path.exists(preextracted_path)) and not os.path.exists(video_path):
                        logging.warning(f"Video/Frames not found for ID: {video_id}. Skipping.")
                        continue

                    model_device = model.device
                    video_tensor = image_processor.preprocess(raw_frames, return_tensors="pt")["pixel_values"].to(model_device).bfloat16()

                    prompt = ""
                    if args.dataset == 'videomme':
                        srt_path = os.path.join(args.srt_root, f"{video_id}.srt") if args.srt_root else None
                        subtitles_text = extract_subtitles_for_keyframes(frame_indices, original_fps, srt_path)
                        ...
                        prompt = f"Based on the video, answer the following question. Only provide the text of the correct option.\n\nQuestion: {question}\n\nOptions:\n{options_str}\n\nAnswer:"
                        ...
                        options_str = "\n".join([f"{chr(65+j)}. {cand}" for j, cand in enumerate(candidates)])
                        prompt = (
                            f"Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects, and the action and pose of persons. Based on your observations, select the best option that accurately addresses the question.\n"
                            f"Question: {question}\n"
                            f"Options:\n{options_str}\n"
                            f"Only give the best option."
                        )

                    input_ids, question_embed = prepare_model_inputs(prompt, question, tokenizer, model, model_device)
            ...
            try:
                if model.config.mm_newline_position == "grid":
                    if video_time > 30:
                        model.config.mm_newline_position = "grid"
                    else:
                        model.config.mm_newline_position = "frame"
                final_input_ids = torch.stack(batch_input_ids, dim=0)
                final_question_embeddings = torch.stack(batch_question_embeddings, dim=0)
                
                custom_dict = {"feature_model": feature_model,
                               "raw_images": batch_video_inputs, 
                               "questions": batch_questions,
                               "domain_rate": args.domain_rate,
                               "question_embedding": final_question_embeddings,
                               "question_id": [sample["question_id"] for sample in valid_batch_samples]} if args.use_way else None
                
                t1_gen = time.time()
                cont = model.generate(
                    final_input_ids,
                    images=batch_video_tensors,
                    modalities=["video"],
                    do_sample=False,
                    temperature=0,
                    max_new_tokens=32,
                ...
                del batch_video_tensors, batch_input_ids
                gc.collect()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run inference on Video QA datasets with a LLaVA-based model.")
    ...
    parser.add_argument('--model-path', type=str, required=True, help="Path to the LLaVA model.(LLava-Video or LLava-Onevision)")
